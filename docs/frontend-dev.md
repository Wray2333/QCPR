...
└─ src/
   ...
   │  │  ├─ TimerPractice.jsx  # 一键计时练习
   │  │  ├─ Timer.jsx          # 计时器展示
   │  │  ├─ ResultForm.jsx     # 结束后填错题数
...
表单 (ManualEntry)
   ...
   │  · 错题数
   ▼
校验（错题数 ≤ 题量、用时 > 0、日期时间合法且不晚于当前）
   ▼
保存 → addRecord({ source: 'manual', ... })
```
...
export default {
  ...
  theme: {
    extend: {
      colors: {
        brand: '#2563eb',      // 主色（蓝）
        success: '#16a34a',    // 达标 / 正确率高
        warning: '#f59e0b',    // 临界
        danger:  '#dc2626',    // 超时 / 正确率低
      },
    },
  },
  plugins: [],
...
将其收敛为工具函数（如 `format.js` 中的 `accuracyColor(rate)`），组件统一调用。
...
| 组件 | 规范 |
|---|---|
| `Card` | 圆角 `rounded-xl`、浅阴影 `shadow-sm`、内边距 `p-4` |
| `Button` | 主按钮 `bg-brand text-white`，次按钮描边；禁用态降透明度 |
| `Tabs` | 选中项底部主色下划线 |
| `EmptyState` | 居中图标 + 文案 + 引导按钮 |
