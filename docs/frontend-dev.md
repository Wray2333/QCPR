...
└─ src/
   ...
   │  │  ├─ Timer.jsx          # 番茄钟圆环计时展示
   │  │  ├─ ResultForm.jsx     # 结束后填错题数（拨轮）
   │  │  ├─ DurationPicker.jsx # 用时分秒双拨轮
   │  │  └─ ManualEntry.jsx    # 手动补录表单（拨轮）
   │  └─ common/
   │     ├─ Button.jsx
   │     ├─ Card.jsx
   │     ├─ Tabs.jsx
   │     ├─ WheelPicker.jsx    # 通用拨轮选择器（scroll-snap）
   │     └─ EmptyState.jsx     # 空数据引导
   ├─ hooks/
...
点击「结束」
   ...
   ▼
填写结果 (ResultForm)
   │  · 错题数用拨轮选择（WheelPicker，0 ~ totalCount），可切手动输入
   │  · 展示用时、自动算出的正确率
   ▼
...
表单 (ManualEntry)
   ...
   │  · 用时（DurationPicker：分/秒双拨轮，可切手动输入）
   │  · 错题数（WheelPicker 拨轮，0 ~ 题量，可切手动输入）
   ▼
...
> **拨轮选择器（WheelPicker / DurationPicker）**：iOS 风格竖向滚动选择器，基于原生 CSS `scroll-snap`（零依赖），当前值吸附居中高亮，支持触摸/滚轮/上下键；**点击中央数值**即切换为手动输入态（主色边框提示），失焦或回车提交——即"选优先、可输可选"。`DurationPicker` 由分、秒两个 `WheelPicker` 组合，对外以总秒数交互，用函数式 setter 更新避免分/秒回调相互覆盖。
...
| `Collapsible` | shadcn（Radix）——`CollapsibleField` 折叠用时/错题拨轮 |
...
| `EmptyState` / `WheelPicker` / `DurationPicker` / `Timer` 圆环 / 底部 TabBar | 无官方对应，自研，样式对齐 shadcn token |
